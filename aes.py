...
if mode == 1:
    ...
    msg = sys.argv[2].encode().decode('unicode_escape').encode("raw_unicode_escape")


aes_key = ""
k = len(str(key))
for i in range(16):
    aes_key = aes_key + str(key)[i % k]

if mode == 1:
    result = AES(aes_key.encode()).encrypt_ctr(msg.encode(), iv)
    decresult = AES(aes_key.encode()).decrypt_ctr(result, iv).decode('utf8')
elif mode == 2:
    result = AES(aes_key.encode()).decrypt_ctr(msg, iv).decode('utf8')
else: result = 'Error'
